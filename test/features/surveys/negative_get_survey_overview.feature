Feature: Must not to GET a Survey Overview with invalid inputs
  The feature must not to get a request with invalid inputs
  at the service /surveys/{surveyId}/overview
  ...
  Scenario Outline: Perform a GET surveys state with an invalid token
    ...
    When I perform a GET  at the service "/surveys/{survey_response._id}/overview"
  ...
  Scenario Outline: Perform a GET surveys state with an invalid survey id
    Given I have the Authorization header
    When I perform a GET  at the service "/surveys/<Survey Response>/overview"
